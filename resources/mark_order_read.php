...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['order_id']) && isset($_SESSION['email'])) {
    ...
    // Get user ID
    $stmt_user = mysqli_prepare($conn, "SELECT user_id FROM users WHERE email = ?");
    ...
    if ($user) {
        ...
        $stmt_update = mysqli_prepare($conn, "UPDATE orders SET notification_status = 'read' WHERE order_id = ? AND user_id = ?");
