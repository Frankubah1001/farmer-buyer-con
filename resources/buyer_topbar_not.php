...
if (isset($_SESSION['email'])) {
    ...
    $sql_select_user = "SELECT buyer_id FROM buyers WHERE email = ?";
    ...
    if ($user = mysqli_fetch_assoc($result_select_user)) {
        ...
        // Count unread orders
        $sql_count_unread = "SELECT COUNT(*) AS orderCount FROM orders WHERE buyer_id = ? AND notification_status = 'unread'";
        $stmt_count = mysqli_prepare($conn, $sql_count_unread);
        mysqli_stmt_bind_param($stmt_count, "i", $buyer_id);
