 ...
 * Handles CRUD operations for grant applications
 */

require_once 'DBcon.php';
require_once 'grant_email_helper.php';
...
function getAllApplications($conn) {
    ...
    $where = ["ga.application_status != 'Deleted'"];
    
    if (!empty($status)) {
        $where[] = "ga.application_status = '$status'";
    }
    
    if (!empty($search)) {
        $where[] = "(u.first_name LIKE '%$search%' OR u.last_name LIKE '%$search%' OR u.email LIKE '%$search%' OR ga.provider_name LIKE '%$search%')";
    }
    
    $whereClause = implode(' AND ', $where);
    
    $countSql = "SELECT COUNT(*) as total FROM grant_applications ga JOIN users u ON ga.user_id = u.user_id WHERE $whereClause";
...
function approveApplication($conn) {
    ...
    $detailsSql = "SELECT 
                ...
                FROM grant_applications ga
                JOIN users u ON ga.user_id = u.user_id
                WHERE ga.application_id = ? AND ga.application_status = 'Pending'";
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            // Send approval email
            $emailSent = sendGrantApprovalEmail(
...
function rejectApplication($conn) {
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            ...
            $emailSent = sendGrantRejectionEmail(
                $applicationData['farmer_email'],
                $applicationData['farmer_name'],
                $applicationData,
                $rejection_reason
...
function getStats($conn) {
    $sql = "SELECT 
                ...
                SUM(CASE WHEN application_status = 'Pending' THEN amount_requested ELSE 0 END) as pending_amount,
                SUM(CASE WHEN application_status = 'Approved' THEN amount_requested ELSE 0 END) as approved_amount
